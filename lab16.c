...
int delete_k(double* ptr_array2, int size);// удаление заданного эл-та, вариант 7 (15-8)
// задание 3
...
double* full_random(double* ptr_array2, int size) {
...
int delete_k(double* ptr_array2, int size){
	...
	for (int i = maxI; i < (size - 1); i++) { // удаление максимального элемента
		ptr_array2[i] = ptr_array2[i + 1];
	}
	for (int i = minI; i < (size - 1); i++) { // удаление минимального элемента
		ptr_array2[i] = ptr_array2[i + 1];
	}
...
double* insert(double* ptr_arr, int *size2) {
	...
	if (ptr_arr == NULL) 
		return NULL;
	double* ptr_arr_n = (double*)realloc(ptr_arr, size_n * sizeof(double)); // перераспределение памяти
	if (ptr_arr_n == NULL) 
		return ptr_arr; // проверка выделенной памяти

	ptr_arr = ptr_arr_n;

	for (int i = size_n - 1; i > minI; --i) {
		ptr_arr[i] = ptr_arr[i - 2]; // сдвиг элементов
	}
