...
void main()
{
    ...
    char data[100]; // буфер для записи строки
    ...
    int data_l = strlen(data); // 2.3
    int otstyp = (80 - data_l) / 2;
    int ost_otstyp = 80 - data_l - otstyp;

    for (int i = 0; i < 10; i++) {
        if (i == 5) {
            // Создаём строку с пробелами
            for (int j = 0; j < otstyp; j++) {
                printf(" ");
            }
            printf("%s", data);
            for (int j = 0; j < ost_otstyp; j++) {
                printf(" ");
            }
            printf("\n");
        }
    ...
    G6 = (GR){ "МАЗ-6501","МАЗ", 25, 2008, 2024 };
